...
2. R script file named 'data_convert_preperation.R' is required mainly to reduce the size
   ...
   the data is too large in its raw form. Use this script to load, wrangle, then save the data.
3. R script file named 'simulation_analysis.R' is the main script to replicate the study. It can only
   be ran once data has been processed in steps 1 and 2. It generates and saves the visuals used in the
   paper. Statistics reported in the paper are usually printed to the Console, rather than saved automatically.
